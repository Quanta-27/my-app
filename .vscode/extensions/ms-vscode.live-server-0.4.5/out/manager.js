...
class Manager extends dispose_1.Disposable {
     ...
     */
    async openPreviewAtFileString(filePath, previewType) {
        if (filePath === '') {
            return this._openPreviewWithNoTarget();
        }
        const workspace = await pathUtil_1.PathUtil.GetWorkspaceFromRelativePath(filePath);
        if (workspace) {
            const file = vscode.Uri.joinPath(workspace.uri, await pathUtil_1.PathUtil.GetValidServerRootForWorkspace(workspace), filePath);
